...
```{r another regression tree}
# The tutorial had already covered simple tree method and both boosting and bagging tree methods, so I googled around and found a "conditional inference tree" method, which basically differs from other tree methods by selecting/splitting variables at each node basing on the results of significance tests (you can see below in the tree model plot that p values are presented at each node). This approach also allow this model to be used directly with minimal pruning.

data(BostonHousing)
train_size <- floor(0.70 * nrow(BostonHousing))
train_pos <- sample(seq_len(nrow(BostonHousing)), size = train_size)
train_regression <- BostonHousing[train_pos,]
test_regression <- BostonHousing[-train_pos,]

ctree_model <- ctree(medv ~ .,
   data=train_regression)
plot(ctree_model, main="Conditional Inference Tree")

ctree_predict = predict(ctree_model, newdata = test_regression)
plot(ctree_predict,test_regression$medv,xlab="Predict values", ylab="True values")

ctree_error = mean((as.matrix(ctree_predict) - as.matrix(test_regression$medv))^2)
cat(sprintf('Mean squared error = %f',ctree_error))
# From the ctree model plot, we can see that the most important predictors for Boston housing value, similar with the boosting tree example in the tutorial, are lstat and rm.
```

2. Attempt both a bagging and boosting method on a reasonable dataset of your choice. Explain the results.
# Bagging tree with Iris dataset
```{r Random Forest iris}
data("iris")
iris$Species = as.factor(iris$Species)
iris = iris[complete.cases(iris), ]
train_size <- floor(0.70 * nrow(iris))
train_pos <- sample(seq_len(nrow(iris)), size = train_size)
train_regression <- iris[train_pos,]
test_regression <- iris[-train_pos,]

# train
iris.rf = randomForest(Species~., data = train_regression, mtry=3, ntree = 50)
iris.rf

# test
pred = predict(iris.rf, newdata=test_regression) 
confusionMatrix(pred, reference = test_regression$Species)
```

# Boosting tree with Iris dataset
```{r Boosting iris}
# train
iris.boost = boosting(Species~., data=train_regression, boos=TRUE, mfinal=3)
summary(iris.boost)

# test
pred = predict(iris.boost, newdata = test_regression)
confusionMatrix(as.factor(pred$class), reference = test_regression$Species)
# The iris data is quite easy to classify, and the two methods performed equally well with about 96% accuracy
```
